...
- [✔] **2026-07-25 HQ-managed upsell prices** — Checkout Upsell settings page now takes a special price per upsell product (Filament Repeater of product + price; blank = normal price). New settings key `upsell.prices` (JSON map product_id → price) alongside the existing `upsell.product_ids`, so old saved selections keep working. Price enforced server-side: `OrderLinePayload::$isUpsell` → `OrderService::upsellPrices()` applied in `place()` + `quote()` (the service always recomputes unit price, so a client-side price alone would have been ignored and the customer charged full price). Cart lines added from the upsell sheet get `is_upsell` + an `upsell:`-prefixed line id so they never merge with the same product bought at normal price. Sheet shows the normal price struck through. Verified: php -l clean, `tsc` clean (only the pre-existing `ssr.tsx:10` error), `npm run build` green. Not committed.

- [✔] **2026-07-25 claimable vouchers in the upsell** — New "Upsell vouchers" section on the same settings page (`upsell.voucher_ids`; spin-only + check-in-only vouchers excluded from the picker). `Controller::upsellVouchers()` returns only vouchers the signed-in customer is eligible for (`isEligibleFor`), hasn't claimed, that pass branch scope + `max_uses` + active date window; guests get an empty list. Sheet renders them above the add-ons with a Claim button (shows "Claim · N pts" for points-cost rewards) and claims via `router.post('/vouchers/{id}/claim', {}, { preserveState, preserveScroll, only: ['flash'] })` so the sheet stays open — the claim controller returns `back()->with('error')`, which Inertia treats as *success*, so the error is read off `flash.error` in `onSuccess`. Claimed vouchers are applied on the checkout page (existing chips). Both entry points now open the sheet when there are products **or** vouchers to offer. Verified with a bootstrapped script against local DB: payload returned the eligible voucher, `quote()` charged 2 × RM1.50 = RM3 for an upsell line vs RM12 normally; test settings rows deleted afterwards (Checkout Upsell had never been configured locally, so nothing was overwritten). Not committed.
